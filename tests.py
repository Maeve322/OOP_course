...
@pytest.fixture
def vacancy():
    return Vacancy("Python Developer", URL="https://hh.tu/1231451512",
                   salary=1000, description="need to super power")
...
def test_init_with_null_salary():
    temp_vacancy = Vacancy("Python Developer", URL="https://hh.tu/1231451512",
                           salary="null", description="need to super power")
    assert temp_vacancy.salary == 0


def test_init_with_dict_salary():
    salary = {"from": 3000, "to": 5000}
    vacancy = Vacancy("Data Scientist", "Machine learning expert", salary,
                      "https://example.com")
    assert vacancy.salary == 4000


def test_init_with_regular_salary():
    vacancy = Vacancy("Product Manager", "Product strategy",
                      6000, "https://example.com")
...
def test_equals_vacancy(vacancy):
    assert vacancy == Vacancy("Python Developer",
                              URL="https://hh.tu/1231451512", salary=1000,
                              description="need to super power")


def test_less_vacancy(vacancy):
    assert vacancy < Vacancy("Python Developer",
                             URL="https://hh.tu/1231451512",
                             salary=50000, description="need to super power")


def test_greater_vacancy(vacancy):
    assert Vacancy("Python Developer",
                   URL="https://hh.tu/1231451512",
                   salary=50000, description="need to super power") > vacancy
...
def test_load_vacancies_success():
    with patch("builtins.open",
               mock_open(read_data='[{"id": 1,"name": "Software Engineer"}]')):
        json_saver = JsonSaver()
        assert json_saver.\
            _JsonSaver__vacancies == [{"id": 1, "name": "Software Engineer"}]
